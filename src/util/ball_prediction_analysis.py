...
from rlbot.flat import BallPrediction, PredictionSlice
...
def find_matching_slice(
    ...
    for coarse_index in range(
        start_index, len(ball_prediction.slices), search_increment
    ):
        if predicate(ball_prediction.slices[coarse_index]):
            for j in range(
                max(start_index, coarse_index - search_increment), coarse_index
            ):
